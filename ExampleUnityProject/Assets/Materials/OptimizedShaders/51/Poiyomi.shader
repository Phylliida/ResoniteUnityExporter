Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/f4a852bebc813254dbe37b08e3e44c71"
{
	Properties
	{
 ...
 #define PROP_GLITTERMASK 
			...
			{
				#if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
				half3 detailTexture = POI2D_SAMPLER_PAN(_DetailTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1), float(0));
				#else
				half3 detailTexture = 0.21763764082 * poiThemeColor(poiMods, float4(1,1,1,1), float(0));
				#endif
				poiFragData.baseColor.rgb *= LerpWhiteTo(detailTexture * float(1.88) * unity_ColorSpaceDouble.rgb, poiMods.detailMask.r * float(0.83));
			}
			void ApplyDetailNormal(inout PoiMods poiMods, inout PoiMesh poiMesh)
			{
				#if defined(PROP_DETAILMASK) || !defined(OPTIMIZER_ENABLED)
				poiMods.detailMask = POI2D_SAMPLER_PAN(_DetailMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rg;
				#else
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.1,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float4(0,0,0,0) +sideMod, float(0), poiMesh, poiCam);
				ddxuv = ddx(uv);
				ddyuv = ddy(uv);
				if (any(fwidth(uv) > .5))
				{
				...
				decalColor = tex2D(_DecalTexture, uv + float4(0,0,0,0) * _Time.x, ddxuv, ddyuv) * float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), float4(1,1,1,1).a);
				#else
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.1,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float4(0,0,0,0) +sideMod, float(0), poiMesh, poiCam);
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(1)), decalColor.a * saturate(float(1) + audioLinkDecalAlpha0));
				if (float(0))
				{
					alphaOverride += 1;
				...
				#endif
				poiFragData.emission += decalColor.rgb * decalColor.a * max(float(0) + audioLinkDecalEmission0, 0);
				#endif
				if (alphaOverride)
				{
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(0.07), float(0), float(0), float(0), float(1), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionColor0 = hueShift(emissionColor0, frac(float(0.789) + float(0) * _Time.x) * float(1));
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
			...
			{
				float2 st = frac(poiMesh.uv[float(0)] + float4(0,0,0,0).xy * _Time.x) * float(1000);
				...
				half3 glitterColor = poiThemeColor(poiMods, float4(1,0.2784314,0.5367146,1), float(0));
				...
				switch(float(0))
				{
					...
					if (float(10) > 0)
					{
						randomRotation = randomFloat3WiggleRange(randoPoint, float(90), float(10));
					}
					else
					{
					...
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.334)), poiCam.viewDir)), float(27)), 0);
					...
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(specAngle, float(27)), 0);
				...
				if (float(0) == 1)
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, finalGlitter * glitterColor * float(8.6), finalGlitter * glitterTexture.a * glitterMask);
					poiFragData.emission += finalGlitter * glitterColor * max(0, (float(8.6) - 1) * glitterTexture.a) * glitterMask;
				}
				else
				{
					poiFragData.emission += finalGlitter * glitterColor * float(8.6) * glitterTexture.a * glitterMask;
				}
			}
			#endif
