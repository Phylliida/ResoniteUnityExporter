Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/62ba501b9ee6ed145875ff91d716f35f"
{
	Properties
	{
 ...
 #define PROP_DISSOLVEDETAILNOISE 
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(0.367), float(0), float(1), float(0.152), float(0), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
				...
				blendMatcap(poiLight, poiFragData, float(0), float(0), float(0), float(1), float(0), matcap2, matcap2Mask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float rim = 1 - smoothstep(min(float(0), rimWidth), rimWidth, viewDotNormal);
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)), float(1.78));
