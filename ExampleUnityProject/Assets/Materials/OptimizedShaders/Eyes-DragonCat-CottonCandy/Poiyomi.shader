Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/7a0398d3bb1bcf7488bea67bda28aa05"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMASK1 
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(2)), decalColor.a * saturate(float(1) + audioLinkDecalAlpha0));
				if (float(0))
				{
					alphaOverride += 1;
				...
				#endif
				poiFragData.emission += decalColor.rgb * decalColor.a * max(float(0.3) + audioLinkDecalEmission0, 0);
				#endif
				if (alphaOverride)
				{
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (float(1))
				{
					emissionStrength0 *= calculateBlinkingEmission(float(0), float(1), float(50), float(0));
				}
			...
			float3 applyEmission1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				{
					emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				}
				#else
				...
				#if defined(PROP_EMISSIONMASK1) || !defined(OPTIMIZER_ENABLED)
				float emissionMask1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask1, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,0.8,0,0.1)) + _Time.x * float4(0,0,0,0));
				#else
