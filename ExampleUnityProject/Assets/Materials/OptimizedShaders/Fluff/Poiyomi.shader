Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/f63e21dd08ddf51428199b95e18a108a"
{
	Properties
	{
 ...
 #define PROP_MATCAP 
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
				matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(poiThemeColor(poiMods, float4(0.02732088,0.01096009,0.01228648,1).rgb, float(0)), float4(0.02732088,0.01096009,0.01228648,1).a);
				#else
				matcap = float4(poiThemeColor(poiMods, float4(0.02732088,0.01096009,0.01228648,1).rgb, float(0)), float4(0.02732088,0.01096009,0.01228648,1).a);
				#endif
				matcap.rgb *= float(1);
				#if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
				...
				if (float(1))
				{
					matcap.rgb = hueShift(matcap.rgb, float(0.733) + _Time.x * float(0));
				}
				blendMatcap(poiLight, poiFragData, float(1), float(0), float(0), float(1), float(0), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.6038274,0.6038274,0.6038274,1).rgb, float(0));
