Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/8763945c2a1d2c4479e2e5ac6aad1d9c"
{
	Properties
	{
 ...
 #define PROP_DISSOLVEDETAILNOISE 
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(0.23), float(0), float(1), float(0), float(1), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)), float(2.8));
