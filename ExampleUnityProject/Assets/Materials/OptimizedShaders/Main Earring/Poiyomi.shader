Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/2db48006da0498445bcb6cb87574b98c"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(1))
				{
					percepRough = GSAA_Filament(poiMesh.normals[1], percepRough, float(0.311), float(0.176));
					if (float(0) == 1 && float(1) > 0)
					{
						percepRough2 = GSAA_Filament(poiMesh.normals[1], percepRough2, float(0.311), float(0.176));
					}
				}
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.03562092,0.03562092,0.03562092,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.03562092,0.03562092,0.03562092,1).a;
