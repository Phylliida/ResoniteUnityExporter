Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/8000e0f5d1288984db9ff8037f56e86a"
{
	Properties
	{
 ...
 #define PROP_DISSOLVEDETAILNOISE 
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#if defined(PROP_DISSOLVETOTEXTURE) || !defined(OPTIMIZER_ENABLED)
				dissolveToTexture = POI2D_SAMPLER_PAN(_DissolveToTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(1,1,1,0).rgb, float(0)), float4(1,1,1,0).a);
				#else
				dissolveToTexture = float4(1,1,1,0);
				#endif
				#if defined(PROP_DISSOLVENOISETEXTURE) || !defined(OPTIMIZER_ENABLED)
				float dissolveNoiseTexture = POI2D_SAMPLER_PAN(_DissolveNoiseTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(5,5,0,0)), float4(0,0,0,0)).r;
				#else
				...
				#if defined(PROP_DISSOLVEDETAILNOISE) || !defined(OPTIMIZER_ENABLED)
				float dissolveDetailNoise = POI2D_SAMPLER_PAN(_DissolveDetailNoise, _MainTex, poiUV(poiMesh.uv[float(0)], float4(5,5,0,0)), float4(0,0,0,0));
				#else
				...
				if (float(1) == 1) // Basic
				{
					...
					edgeAlpha = remapClamped(da + float(0.02), da, noise) * (1 - dissolveAlpha);
				}
				...
				poiFragData.emission += lerp(0, dissolveToTexture * float(0), dissolveAlpha) + lerp(0, edgeColor.rgb * float(5.2), smoothstep(0, 1 - float(0) * .99999999999, edgeAlpha));
				#endif
			}
			#endif
			//ifex float(1)==0
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(1), float(0), float(0), float(1), float(1), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(float(0))
				{
					...
					if (float(10) > 0)
					{
						randomRotation = randomFloat3WiggleRange(randoPoint, float(5.8), float(10));
					}
					else
					{
						randomRotation = randomFloat3Range(randoPoint, float(5.8));
					}
					float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.8)), poiCam.viewDir)), float(390)), 0);
					...
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(specAngle, float(390)), 0);
				...
				if (float(0) == 1)
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, finalGlitter * glitterColor * float(0.8), finalGlitter * glitterTexture.a * glitterMask);
					poiFragData.emission += finalGlitter * glitterColor * max(0, (float(0.8) - 1) * glitterTexture.a) * glitterMask;
				}
				else
				{
					poiFragData.emission += finalGlitter * glitterColor * float(0.8) * glitterTexture.a * glitterMask;
				}
			}
			#endif
			//ifex float(1)==0 && float(0)==0
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(1))
				{
					percepRough = GSAA_Filament(poiMesh.normals[1], percepRough, float(0.09), float(0.08));
					if (float(0) == 1 && float(1) > 0)
					{
						percepRough2 = GSAA_Filament(poiMesh.normals[1], percepRough2, float(0.09), float(0.08));
					}
				}
				...
				float3 reflCol = GetReflections(poiCam, poiLight, poiMesh, roughness, float(1), float(0), _MochieReflCube, poiCam.reflectionDir);
