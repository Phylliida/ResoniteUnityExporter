Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/5b6c26966ff861a4498b6111eb69b652"
{
	Properties
	{
		[HideInInspector] shader_master_label ("<color=#E75898ff>Poiyomi 8.0.420</color>", Float) = 0
 ...
 #define PROP_GLITTERMASK 
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				decalColor = tex2D(_DecalTexture, uv + float4(0,0,0,0) * _Time.x, ddxuv, ddyuv) * float4(poiThemeColor(poiMods, float4(0.5379038,0.3515327,1,1).rgb, float(0)), float4(0.5379038,0.3515327,1,1).a);
				#else
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float4(0,0,0,0) +sideMod, float(0), poiMesh, poiCam);
				decalColor = float4(poiThemeColor(poiMods, float4(0.5379038,0.3515327,1,1).rgb, float(0)), float4(0.5379038,0.3515327,1,1).a);
				#endif
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(18)), decalColor.a * saturate(float(1) + audioLinkDecalAlpha0));
				if (float(0))
				{
					alphaOverride += 1;
				...
				#endif
				poiFragData.emission += decalColor.rgb * decalColor.a * max(float(1.8) + audioLinkDecalEmission0, 0);
				#endif
				//ifex float(1)==0
				#ifdef GEOM_TYPE_BRANCH_DETAIL
				decalScale = float4(1,1,0,0);
				...
				decalColor.a *= applyTilingClipping(float(0), uv) * decalMask[float(1)];
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(18)), decalColor.a * saturate(float(1) + audioLinkDecalAlpha1));
				if (float(0))
				{
					alphaOverride += 1;
				...
				#endif
				poiFragData.emission += decalColor.rgb * decalColor.a * max(float(1) + audioLinkDecalEmission1, 0);
				#endif
				//ifex float(1)==0
				#ifdef GEOM_TYPE_FROND
				decalScale = float4(1,1,0,0);
				...
				decalColor = tex2D(_DecalTexture2, uv + float4(0,0,0,0) * _Time.x, ddxuv, ddyuv) * float4(poiThemeColor(poiMods, float4(1,0,0.6243634,1).rgb, float(0)), float4(1,0,0.6243634,1).a);
				#else
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float4(0,0,0,0) + sideMod, float(0), poiMesh, poiCam);
				decalColor = float4(poiThemeColor(poiMods, float4(1,0,0.6243634,1).rgb, float(0)), float4(1,0,0.6243634,1).a);
				#endif
				decalColor.rgb = decalHueShift(float(0), decalColor.rgb, float(0) + poiLight.nDotV * float(0), float(0));
				decalColor.a *= applyTilingClipping(float(0), uv) * decalMask[float(2)];
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(float(1) + audioLinkDecalAlpha2));
				if (float(0))
				{
					alphaOverride += 1;
					decalAlpha = lerp(decalAlpha, min(decalAlpha, decalColor.a), decalMask[float(2)]);
				}
				...
				#endif
				poiFragData.emission += decalColor.rgb * decalColor.a * max(float(5) + audioLinkDecalEmission2, 0);
				#endif
				//ifex float(1)==0
				#ifdef DEPTH_OF_FIELD_COC_VIEW
				decalScale = float4(1,1,0,0);
				...
				decalColor = tex2D(_DecalTexture3, uv + float4(0,0,0,0) * _Time.x, ddxuv, ddyuv) * float4(poiThemeColor(poiMods, float4(1,0,0.9657812,1).rgb, float(0)), float4(1,0,0.9657812,1).a);
				#else
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float4(0,0,0,0) + sideMod, float(0), poiMesh, poiCam);
				decalColor = float4(poiThemeColor(poiMods, float4(1,0,0.9657812,1).rgb, float(0)), float4(1,0,0.9657812,1).a);
				#endif
				decalColor.rgb = decalHueShift(float(0), decalColor.rgb, float(0) + poiLight.nDotV * float(0), float(0));
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(8)), decalColor.a * saturate(float(1) + audioLinkDecalAlpha3));
				if (float(0))
				{
					alphaOverride += 1;
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(1))
				{
					lns.x = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.1));
					lns.y = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.3));
					lns.z = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.25), float(0.1));
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.1), float(0));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.1));
					lns.y = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.3));
					lns.z = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.25), float(0.1));
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.1), float(0));
				}
				lns = saturate(lns);
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(float(1))
				{
					case 0:
					{
						...
						matcapUV = noSknewViewNormal.rg * float(0.5) + 0.5;
						break;
					}
					case 1:
					{
						...
						matcapUV = float2(dot(worldViewRight, normal0), dot(worldViewUp, normal0)) * float(0.5) + 0.5;
						break;
					}
					case 2:
					{
				...
				if (float(0))
				{
					matcapMask = lerp(0, matcapMask * TPSBufferedDepth(poiMesh.localPos, poiMesh.vertexColor), float(1));
				}
				#endif
				poiFragData.alpha *= lerp(1, matcap.a, matcapMask * float(0));
				if (float(1))
				{
					matcap.rgb = hueShift(matcap.rgb, _MatcapHueShift + _Time.x * float(0));
				}
				blendMatcap(poiLight, poiFragData, float(1), float(0), float(1), float(1), float(0), matcap, matcapMask, float(2), float(0)
				#ifdef POI_BLACKLIGHT
				...
				switch(float(1))
				{
					case 0:
					{
						...
						float3 NormalBlend_MatCapUV_Base = (mul(UNITY_MATRIX_V, float4(poiCam.viewDir, 0)).rgb * float3(-1, -1, 1)) + float3(0, 0, 1);
						float3 noSknewViewNormal = NormalBlend_MatCapUV_Base * dot(NormalBlend_MatCapUV_Base, NormalBlend_MatCapUV_Detail) / NormalBlend_MatCapUV_Base.b - NormalBlend_MatCapUV_Detail;
						matcapUV = noSknewViewNormal.rg * float(0.348) + 0.5;
						break;
					}
					case 1:
					{
						float3 worldViewUp = normalize(float3(0, 1, 0) - poiCam.viewDir * dot(poiCam.viewDir, float3(0, 1, 0)));
						float3 worldViewRight = normalize(cross(poiCam.viewDir, worldViewUp));
						matcapUV = float2(dot(worldViewRight, normal1), dot(worldViewUp, normal1)) * float(0.348) + 0.5;
						break;
					}
					case 2:
					{
				...
				if (float(0))
				{
					matcap2Mask = lerp(matcap2Mask, 1, TPSBufferedDepth(poiMesh.localPos, poiMesh.vertexColor) * float(1));
				}
				#endif
				poiFragData.alpha *= lerp(1, matcap2.a, matcap2Mask * float(0));
				if (float(1))
				{
					matcap2.rgb = hueShift(matcap2.rgb, _Matcap2HueShift + _Time.x * float(0));
				}
				blendMatcap(poiLight, poiFragData, float(0), float(0), float(0), float(0), float(0), matcap2, matcap2Mask, float(2), float(0)
				#ifdef POI_BLACKLIGHT
			...
			#endif
			//ifex float(1)==0 && float(1)==0 && float(0)==0 && float(0)==0
			...
			{
				float3 emission0 = 0;
				float emissionStrength0 = float(2.5);
				...
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(1)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				}
				#else
			...
			float3 applyEmission1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionStrength1 = float(0.2);
				float glowInTheDarkMultiplier1 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0), poiLight);
				#if defined(PROP_EMISSIONMAP1) || !defined(OPTIMIZER_ENABLED)
				if (!float(0))
				{
					emissionColor1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				}
				else
				{
					emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(2)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				}
				#else
				emissionColor1 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				#endif
				if (float(1))
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE1) || !defined(OPTIMIZER_ENABLED)
						emissionStrength1 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve1, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)) + (dot(pos1, float4(0,-10,0,0)) * float(20)) + _Time.x * float(10));
						#endif
					}
					else
					{
				...
				if (float(1))
				{
					emissionStrength1 *= calculateBlinkingEmission(float(20), float(0), float(50), float(0));
				}
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.3691636,0.08875599,0.07605494,1).rgb, float(0)), float4(0.3691636,0.08875599,0.07605494,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.3691636,0.08875599,0.07605494,1).rgb, float(0)), float4(0.3691636,0.08875599,0.07605494,1).a);
				#endif
				...
				float rim = 1 - smoothstep(min(float(0.198), rimWidth), rimWidth, viewDotNormal);
				rim *= float4(0.3691636,0.08875599,0.07605494,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.3691636,0.08875599,0.07605494,1).rgb, float(0)));
			...
			{
				float2 st = frac(poiMesh.uv[float(0)] + float4(0,0,0,0).xy * _Time.x) * float(2000);
				...
				for (int j = -1; j <= 1; j++)
				{
					for (int i = -1; i <= 1; i++)
					{
						...
						float2 rando = pos;
						pos = 0.5 + 0.5 * sin(float(0.247) * 6.2831 * pos);
				...
				switch(float(0))
				{
					...
					if (float(10) > 0)
					{
						randomRotation = randomFloat3WiggleRange(randoPoint, float(17.8), float(10));
					}
					else
					{
						randomRotation = randomFloat3Range(randoPoint, float(17.8));
					}
					float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
					finalGlitter = lerp(0, float(0.107) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0)), poiCam.viewDir)), float(19)), 0);
					finalGlitter *= glitterAlpha;
					break;
					case 1:
					float offset = random(randoPoint);
					float brightness = sin((_Time.x + offset) * float(10)) * float(20) - (float(20) - 1);
					finalGlitter = max(float(0.107) * glitterAlpha, brightness * glitterAlpha * smoothstep(0, 1, 1 - m_dist * float(0.08) * 10));
					break;
					case 2:
					if (float(10) > 0)
					{
					...
					finalGlitter = lerp(0, float(0.107) * glitterAlpha, glitterAlpha) + max(pow(specAngle, float(19)), 0);
					glitterColor *= poiLight.directColor;
					finalGlitter *= glitterAlpha;
					break;
				}
				glitterColor *= lerp(1, poiFragData.baseColor, float(0));
				#if defined(PROP_GLITTERCOLORMAP) || !defined(OPTIMIZER_ENABLED)
				...
				float glitterMask = 1;
				#endif
				glitterMask *= lerp(1, poiLight.rampedLightMap, float(0.471));
				#ifdef POI_BLACKLIGHT
				...
				if (float(0) == 1)
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, finalGlitter * glitterColor * float(0.6), finalGlitter * glitterTexture.a * glitterMask);
					poiFragData.emission += finalGlitter * glitterColor * max(0, (float(0.6) - 1) * glitterTexture.a) * glitterMask;
				}
				else
				{
					poiFragData.emission += finalGlitter * glitterColor * float(0.6) * glitterTexture.a * glitterMask;
				}
			}
			#endif
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.9162945,0.8727582,0.830453,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.9162945,0.8727582,0.830453,1).a;
