...
namespace System.IO.Pipes
{
    ...
    public sealed partial class NamedPipeServerStreamDotNet : PipeStreamDotNet
    {
                ...
                HandleInheritability inheritability)
        {
            ...
            PipeSecurityDotNet PipeSecurityDotNet = null;
            ...
            if (IsCurrentUserOnly)
            {
                using (WindowsIdentity currentIdentity = WindowsIdentity.GetCurrent())
                {
                    ...
                    PipeAccessRuleDotNet rule = new PipeAccessRuleDotNet(identifier, PipeAccessRightsDotNet.FullControl, AccessControlType.Allow);
                    PipeSecurityDotNet = new PipeSecurityDotNet();

                    PipeSecurityDotNet.AddAccessRule(rule);
                    PipeSecurityDotNet.SetOwner(identifier);
                }
            ...
            try
            {
                Interop.Kernel32.SECURITY_ATTRIBUTES secAttrs = PipeStreamDotNet.GetSecAttrs(inheritability, PipeSecurityDotNet, ref pinningHandle);
        ...
        private unsafe Task WaitForConnectionCoreAsync(CancellationToken cancellationToken)
        {
            ...
            if (!IsAsync)
            {
                throw new InvalidOperationException(Strings.InvalidOperation_PipeNotAsync);
            }

            var completionSource = new ConnectionCompletionSourceDotNet(this);
