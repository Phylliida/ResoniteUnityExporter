...
namespace System.IO.Pipes
{
    ...
    public sealed partial class NamedPipeServerStreamDotNet : PipeStreamDotNet
    {
        ...
        public NamedPipeServerStreamDotNet(String pipeName)
            : this(pipeName, PipeDirection.InOut, 1, PipeTransmissionMode.Byte, PipeOptions.None, 0, 0, HandleInheritability.None)
        {
        }

        public NamedPipeServerStreamDotNet(String pipeName, PipeDirection direction)
            : this(pipeName, direction, 1, PipeTransmissionMode.Byte, PipeOptions.None, 0, 0, HandleInheritability.None)
        {
        }

        public NamedPipeServerStreamDotNet(String pipeName, PipeDirection direction, int maxNumberOfServerInstances)
        ...
        public NamedPipeServerStreamDotNet(String pipeName, PipeDirection direction, int maxNumberOfServerInstances, PipeTransmissionMode transmissionMode)
            : this(pipeName, direction, maxNumberOfServerInstances, transmissionMode, PipeOptions.None, 0, 0, HandleInheritability.None)
        {
        }

        public NamedPipeServerStreamDotNet(String pipeName, PipeDirection direction, int maxNumberOfServerInstances, PipeTransmissionMode transmissionMode, PipeOptions options)
            : this(pipeName, direction, maxNumberOfServerInstances, transmissionMode, options, 0, 0, HandleInheritability.None)
        {
        }

        public NamedPipeServerStreamDotNet(String pipeName, PipeDirection direction, int maxNumberOfServerInstances, PipeTransmissionMode transmissionMode, PipeOptions options, int inBufferSize, int outBufferSize)
        ...
        /// 1 and 254 (Windows)/greater than 1 (Unix), or use NamedPipeServerStreamDotNet.MaxAllowedServerInstances to use the 
        ...
        /// <param name="PipeSecurityDotNet">PipeSecurityDotNet, or null for default security descriptor</param>
        ...
        private NamedPipeServerStreamDotNet(String pipeName, PipeDirection direction, int maxNumberOfServerInstances,
            ...
            : base(direction, transmissionMode, outBufferSize)
        {
            ...
            {
                throw new ArgumentException(Strings.Argument_NeedNonemptyPipeName);
            }
            if ((options & ~(PipeOptions.WriteThrough | PipeOptions.Asynchronous | PipeOptions.CurrentUserOnly)) != 0)
            {
                throw new ArgumentOutOfRangeException(nameof(options), Strings.ArgumentOutOfRange_OptionsInvalid);
            }
            if (inBufferSize < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(inBufferSize), Strings.ArgumentOutOfRange_NeedNonNegNum);
            }
            if ((maxNumberOfServerInstances < 1 || maxNumberOfServerInstances > 254) && (maxNumberOfServerInstances != MaxAllowedServerInstances))
            {
                ...
                throw new ArgumentOutOfRangeException(nameof(maxNumberOfServerInstances), Strings.ArgumentOutOfRange_MaxNumServerInstances);
            }
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(inheritability), Strings.ArgumentOutOfRange_HandleInheritabilityNoneOrInheritable);
            }
        ...
        // Create a NamedPipeServerStreamDotNet from an existing server pipe handle.
        public NamedPipeServerStreamDotNet(PipeDirection direction, bool isAsync, bool isConnected, SafePipeHandle safePipeHandle)
            : base(direction, PipeTransmissionMode.Byte, 0)
        {
            ...
            {
                throw new ArgumentException(Strings.Argument_InvalidHandle, nameof(safePipeHandle));
            }
        ...
        public System.IAsyncResult BeginWaitForConnection(AsyncCallback callback, object state) =>
            TaskToApmDotNet.Begin(WaitForConnectionAsync(), callback, state);

        public void EndWaitForConnection(IAsyncResult asyncResult) =>
            TaskToApmDotNet.End(asyncResult);
        ...
        private void CheckConnectOperationsServer()
        {
            ...
            {
                throw new IOException(Strings.IO_PipeBroken);
            }
        }
        ...
        private void CheckDisconnectOperations()
        {
            ...
            {
                throw new InvalidOperationException(Strings.InvalidOperation_PipeNotYetConnected);
            }
            if (State == PipeState.Disconnected)
            {
                throw new InvalidOperationException(Strings.InvalidOperation_PipeAlreadyDisconnected);
            }
            if (InternalHandle == null && CheckOperationsRequiresSetHandle)
            {
                throw new InvalidOperationException(Strings.InvalidOperation_PipeHandleNotSet);
            }
    ...
    // (see NamedPipeServerStreamDotNet.RunAsClient).
