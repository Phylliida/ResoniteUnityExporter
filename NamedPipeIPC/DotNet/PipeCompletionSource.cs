...
namespace System.IO.Pipes
{
    internal abstract unsafe class PipeCompletionSourceDotNet<TResult> : TaskCompletionSource<TResult>
    {
        ...
        protected PipeCompletionSourceDotNet(ThreadPoolBoundHandle handle, ReadOnlyMemory<byte> bufferToPin)
        ...
        internal void RegisterForCancellation(CancellationToken cancellationToken)
        {
            ...
            if (cancellationToken.CanBeCanceled && Overlapped != null)
            {
                ...
                if (state == NoResult)
                {
                    ...
                    _cancellationRegistration = cancellationToken.Register(thisRef => ((PipeCompletionSourceDotNet<TResult>)thisRef).Cancel(), this);
