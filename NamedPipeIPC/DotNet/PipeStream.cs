...
namespace System.IO.Pipes
{
    public abstract partial class PipeStreamDotNet : Stream
    {
        ...
        protected PipeStreamDotNet(PipeDirection direction, int bufferSize)
        {
            if (direction < PipeDirection.In || direction > PipeDirection.InOut)
            {
                throw new ArgumentOutOfRangeException(nameof(direction), Strings.ArgumentOutOfRange_DirectionModeInOutOrInOut);
            }
            if (bufferSize < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(bufferSize), Strings.ArgumentOutOfRange_NeedNonNegNum);
            }

            Init(direction, PipeTransmissionMode.Byte, bufferSize);
        }

        protected PipeStreamDotNet(PipeDirection direction, PipeTransmissionMode transmissionMode, int outBufferSize)
        {
            if (direction < PipeDirection.In || direction > PipeDirection.InOut)
            {
                throw new ArgumentOutOfRangeException(nameof(direction), Strings.ArgumentOutOfRange_DirectionModeInOutOrInOut);
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(transmissionMode), Strings.ArgumentOutOfRange_TransmissionModeByteOrMsg);
            }
            if (outBufferSize < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(outBufferSize), Strings.ArgumentOutOfRange_NeedNonNegNum);
            }
        ...
        // Once a PipeStreamDotNet has a handle ready, it should call this method to set up the PipeStreamDotNet.  If
        ...
        {
            if (_isAsync)
                return TaskToApmDotNet.Begin(ReadAsync(buffer, offset, count, CancellationToken.None), callback, state);
            else
        ...
        {
            if (_isAsync)
                return TaskToApmDotNet.End<int>(asyncResult);
            else
        ...
        {
            if (_isAsync)
                return TaskToApmDotNet.Begin(WriteAsync(buffer, offset, count, CancellationToken.None), callback, state);
            else
        ...
        {
            if (buffer == null)
                throw new ArgumentNullException(nameof(buffer), Strings.ArgumentNull_Buffer);
            if (offset < 0)
                throw new ArgumentOutOfRangeException(nameof(offset), Strings.ArgumentOutOfRange_NeedNonNegNum);
            if (count < 0)
        ...
        // intended to be called by custom PipeStreamDotNet concrete children
        ...
        public bool IsMessageComplete
        {
            ...
            get
            {
                ...
                {
                    throw new InvalidOperationException(Strings.InvalidOperation_PipeDisconnected);
                }
                ...
                {
                    throw new InvalidOperationException(Strings.InvalidOperation_PipeReadModeNotMessage);
                }
