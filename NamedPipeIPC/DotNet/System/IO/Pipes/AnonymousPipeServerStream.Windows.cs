...
namespace System.IO.Pipes
{
    ...
    public sealed partial class AnonymousPipeServerStreamDotNet : PipeStreamDotNet
    {
        ...
        [SecurityCritical]
        private void Create(PipeDirectionDotNet direction, HandleInheritability inheritability, int bufferSize)
        {
            Debug.Assert(direction != PipeDirectionDotNet.InOut, "Anonymous pipe direction shouldn't be InOut");
            ...
            {
                if (direction == PipeDirectionDotNet.In)
                {
                    bSuccess = Interop.Kernel32.CreatePipe(out serverHandle, out _clientHandle, secAttr.ptr, bufferSize);
                }
                else
                {
                    bSuccess = Interop.Kernel32.CreatePipe(out _clientHandle, out serverHandle, secAttr.ptr, bufferSize);
                }
