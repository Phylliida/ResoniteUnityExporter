...
namespace System.IO.Pipes
{
    public abstract partial class PipeStreamDotNet : Stream
    {
        ...
        private PipeTransmissionModeDotNet _readMode;
        private PipeTransmissionModeDotNet _transmissionMode;
        private PipeDirectionDotNet _pipeDirection;
        private int _outBufferSize;
        private PipeState _state;

        protected PipeStreamDotNet(PipeDirectionDotNet direction, int bufferSize)
        {
            if (direction < PipeDirectionDotNet.In || direction > PipeDirectionDotNet.InOut)
            {
            ...
            Init(direction, PipeTransmissionModeDotNet.Byte, bufferSize);
        }

        protected PipeStreamDotNet(PipeDirectionDotNet direction, PipeTransmissionModeDotNet transmissionMode, int outBufferSize)
        {
            if (direction < PipeDirectionDotNet.In || direction > PipeDirectionDotNet.InOut)
            {
                throw new ArgumentOutOfRangeException(nameof(direction), SR.ArgumentOutOfRange_DirectionModeInOutOrInOut);
            }
            if (transmissionMode < PipeTransmissionModeDotNet.Byte || transmissionMode > PipeTransmissionModeDotNet.Message)
            {
        ...
        private void Init(PipeDirectionDotNet direction, PipeTransmissionModeDotNet transmissionMode, int outBufferSize)
        {
            ...
            if ((_pipeDirection & PipeDirectionDotNet.In) != 0)
            {
                _canRead = true;
            }
            if ((_pipeDirection & PipeDirectionDotNet.Out) != 0)
            {
                _canWrite = true;
            }
        ...
        public bool IsMessageComplete
        {
            ...
            get
            {
                ...
                if (_readMode != PipeTransmissionModeDotNet.Message)
                {
