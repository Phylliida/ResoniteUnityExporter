            ...
            switch (errorCode)
            {
                case Interop.Errors.ERROR_FILE_NOT_FOUND:
                    if (path.Length == 0)
                        ...
                        return new FileNotFoundException(String.Format(SR.IO_FileNotFound_FileName, path), path);

                case Interop.Errors.ERROR_PATH_NOT_FOUND:
                    if (path.Length == 0)
                        return new DirectoryNotFoundException(SR.IO_PathNotFound_NoPathName);
                    else
                        return new DirectoryNotFoundException(String.Format(SR.IO_PathNotFound_Path, path));

                case Interop.Errors.ERROR_ACCESS_DENIED:
                    if (path.Length == 0)
                        return new UnauthorizedAccessException(SR.UnauthorizedAccess_IODenied_NoPathName);
                    else
                        return new UnauthorizedAccessException(String.Format(SR.UnauthorizedAccess_IODenied_Path, path));
                ...
                case Interop.Errors.ERROR_SHARING_VIOLATION:
                    if (path.Length == 0)
                        ...
                        return new IOException(String.Format(SR.IO_SharingViolation_File, path), MakeHRFromErrorCode(errorCode));
