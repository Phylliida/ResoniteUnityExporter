...
namespace BepuPhysics.CollisionDetection
{
    ...
    public partial class NarrowPhase<TCallbacks>
    {
        ...
        {
            BufferPool pool;
            struct PendingConstraint<TBodyHandles, TDescription, TContactImpulses> where TDescription : struct, IConstraintDescription<TDescription>
            {
                ...
                where TDescription : struct, IConstraintDescription<TDescription>
            {
                if (list.Buffer.Allocated)
                {
                    ...
                    for (int i = 0; i < list.Count; ++i)
                    {
                        ...
                        //Unsafe.AsPointer is not a GC hole here; it's coming from struct memory.
